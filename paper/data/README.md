...
| `scalar_ablation_rtx4070.csv` | `scripts/ablate_scalar_control.py` | RTX 4070 | the causal tile-vs-scalar cliff inside Triton |
| `regret_multiseed_rtx4070.csv` | `scripts/regret_multiseed.py` | RTX 4070 | that the headline regret is not a single-seed artifact |
...
not; the CUDA DFA knee tracks L2 capacity), not the absolute numbers.
...
Every driver derives its own output filename, so a run on a different GPU cannot silently
overwrite a file whose name still claims an RTX 4070. Reproducing the *figures* needs no
GPU at all:
